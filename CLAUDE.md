...
domain tags) is in **[[AI-Governance-LLM-wiki/project-particulars]]** in this folder. Read that file
...
acronym: "XYZ"       # definitions only, optional — the standard abbreviation (e.g. "ASL", "GPAI")
...
**Wikilink display policy:** Always write wikilinks with an explicit alias so
the path is never shown as link text in the web viewer:

  `[[wiki/definitions/frontier-ai|Frontier AI]]`      ✓
  `[[wiki/definitions/frontier-ai]]`                  ✗

This applies to all wikilinks in body text and in `related:` frontmatter values.
The alias should match the page's `title:` frontmatter field exactly where
possible, or be a short readable label otherwise. Raw-file links
(`[[raw/filename.md]]`) are exempt — they are display-only references.
...
2. For each new file:
   ...
   d. Identify new technical vocabulary: create wiki/definitions/<slug>.md
      for terms central enough to warrant hover-link definitions. If the term
      has a standard acronym used in the field (e.g. "ASL", "GPAI", "RMF"),
      add `acronym: "XYZ"` to its frontmatter — the viewer uses this to
      auto-expand first occurrences of the acronym across all pages.
